...
def average_models(server: bool, model_list, datasizes, cut_layer_list):
    ...
    for layer_idx, (layer_name, layer) in enumerate(
        model_list[split_idx].model.named_children()
    ):
...
def custom_model_avg(
    server: bool, state_dicts, data_size, cut_layer_list, base_model_class, config
):
    """
    ...
    Args:
        ...
        cut_layer_list (list): List of cut layers for each model.
        base_model_class: The base model class to use for loading state dicts (e.g., ResNet18).
        config (dict): Configuration dict containing 'logits' and other model parameters.
    ...
    for state_dict in state_dicts:
        defn = base_model_class(config)
        defn.load_state_dict(state_dict)
        trained_models.append(defn)

    avg_model, avg_exposure = average_models(
        server, trained_models, data_size, cut_layer_list
    )

    return avg_model, avg_exposure


def combine_fed_avg_models(
    client_fedavg,
    client_exposure,
    server_fedavg,
    server_exposure,
    ...
    Args:
        ...
        base_model_class: The base model class to use for creating the combined model.
        config (dict): Configuration dict containing 'logits' and other model parameters.

    """
    combined = base_model_class(config)
    ...
    for i, key in enumerate(client_fedavg.keys()):
        ...
        state_dict[key] = (
            client_fedavg[key] * c_exposure + server_fedavg[key] * s_exposure
        ) / total

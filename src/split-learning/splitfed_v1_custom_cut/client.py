...
class Runner:
    ...
    def run(self):
        ...
        metrics["initial loading time"] = (
            initial_loading_end_time - initial_loading_start_time
        )
        ...
        for r in range(num_rounds):
            ...
            for epoch in range(num_epochs):
                ...
                metrics["epoch"]["running time"] = (
                    epoch_running_end - epoch_running_start
                )
                ...
                metrics["round"]["server work time"] += metrics["epoch"][
                    "server work time"
                ]
                ...
                epoch_training_str = (
                    f"Training time: {metrics['epoch']['training time']}"
                )
                ...
                metrics["round"]["recv from split"] += metrics["epoch"][
                    "recv from split"
                ]
                # EPOCH OVER
            ...
            logging.info(
                "Size of model weights (before) in bytes is: %s", (getsizeof(weights))
            )
            logging.info(
                "Size of model weights (after) in bytes is: %s",
            ...
            logging.info(
                "SEND_WEIGHTS_COMMUNICATION_TIME = {:.3f}".format(send_weights_time)
            )
            ...
            metrics["round"]["fed wait time"] = (
                weights_waiting_time_end - weights_waiting_time_start
            )
            ...
            round_sent_to_servers = (
                metrics["round"]["sent to fed"] + metrics["round"]["sent to split"]
            )
            round_rcvd_from_servers = (
                metrics["round"]["recv from fed"] + metrics["round"]["recv from split"]
            )
            ...
            round_server_work_str = (
                f"Server work time: {metrics['round']['server work time']}"
            )
            ...
            round_init_str = (
                f"Round Initialization Time: {metrics['round']['init time']}"
            )

            round_sent_to_split_str = (
                f"Data sent to Split Server: {metrics['round']['sent to split']} bytes"
            )
            round_sent_to_fed_str = (
                f"Data sent to Fed Server: {metrics['round']['sent to fed']} bytes"
            )
        ...
        total_sent_split_str = (
            f"Data sent to Split Server: {metrics['sent to split']} bytes"
        )
        total_recv_split_str = (
            f"Data received from Split Server: {metrics['recv from split']} bytes"
        )
        ...
        total_recv_fed_str = (
            f"Data received from Fed Server: {metrics['recv from fed']} bytes"
        )

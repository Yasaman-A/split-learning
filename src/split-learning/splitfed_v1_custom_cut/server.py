    ...
    def run(self):
        ...
        def main():
            ...
            for r in range(num_rounds):
                ...
                for client_num in range(client_total):
                    fed_iters = int(fed_socket.recv().decode())
                    print(fed_iters)
                    fed_socket.send(b"a")
                    ...
                    for i, acc in enumerate(per_class_accuracy):
                        print(f"{i}\t{acc*100:.4f} ({correct_per_class[i]}/{total_per_class[i]})")
                        logging.info(f"{i}\t{acc*100:.4f} ({correct_per_class[i]}/{total_per_class[i]})")
                    print(f"Total Accuracy on client {client_num}'s dataset for round {r}: {accuracy}")
                    logging.info(f"Total Accuracy on client {client_num}'s set for round{r}: {accuracy}")

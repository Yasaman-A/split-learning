...
"""
Quantity Skew Implementation for Split-Learning Framework

This module creates quantity-skewed data distributions from CIFAR-10 dataset using FedArtML.
Quantity skew occurs when clients have different amounts of data, creating heterogeneity
in the data size available to each client.
...
def create_quantity_skew_with_fedartml(
    data, labels, n_clients, alpha_quant_split=1.0, verbose=True
):
    """
    Create quantity skew using FedArtML library with dirichlet method.
    ...
    print(
        f"Creating quantity skew using FedArtML with alpha_quant_split: {alpha_quant_split}"
    )

    # Instantiate SplitAsFederatedData object
    my_federater = SplitAsFederatedData(random_state=0)

    # Create federated dataset with quantity skew using dirichlet method
    clients_glob_dic, list_ids_sampled_dic, miss_class_per_node, distances = (
        my_federater.create_clients(
            ...
            quant_skew_method="dirichlet",  # Use dirichlet for quantity skew
    ...
    if verbose:
        ...
        print("\nQuantity Distribution (should be skewed for quantity skew):")
...
def get_data_loaders_quantity_skew(
    nclients, batch_size, alpha_quant_split=1.0, verbose=True
):
    """
    Create data loaders with quantity skew using FedArtML.
    ...
    # Create quantity-skewed split using FedArtML
...
def run(alpha_quant_split, num_clients, batch_size):
    ...
    """
    print(f"Creating quantity-skewed data for {num_clients} clients using FedArtML...")
    print(f"Alpha for quantity split: {alpha_quant_split}")

    # Create data loaders with quantity skew using FedArtML
    train_loader, test_loader, distances = get_data_loaders_quantity_skew(
...
#         description="Create quantity-skewed federated data using FedArtML"
#     )
#     parser.add_argument(

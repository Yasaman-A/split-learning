...
def split_image_data_realwd(data, labels, n_clients=100, verbose=True):
    ...
    def print_split(clients_split):
        ...
        for i, client in enumerate(clients_split):
            split = np.sum(
                client[1].reshape(1, -1) == np.arange(n_labels).reshape(-1, 1), axis=1
            )
...
def split_image_data(
    data, labels, n_clients=100, classes_per_client=10, shuffle=True, verbose=True
):
    """
...
def get_data_loaders(nclients, batch_size, classes_pc=10, verbose=True):
    ...
    client_loaders = [
        torch.utils.data.DataLoader(
            custom_image_dataset.CustomImageDataset(x, y, transforms_train),
...
def run(classes_pc, num_clients, batch_size):
    ...
    train_loader, test_loader = get_data_loaders(
        classes_pc=classes_pc, nclients=num_clients, batch_size=batch_size, verbose=True
    )

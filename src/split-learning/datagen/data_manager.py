...
# ============================================
# *** ADDED: MedMNIST dataset imports ***
# ============================================
from medmnist import DermaMNIST, OrganAMNIST, OrganCMNIST, OrganSMNIST
import medmnist


# ============================================
# *** ADDED: MedMNIST Dataset Getters ***
# ============================================
def get_dermamnist():
    download_flag = True
    train = DermaMNIST(split="train", download=download_flag, root="./data")
    test = DermaMNIST(split="test", download=download_flag, root="./data")
    return train, test


def get_organmnist_axial():
    download_flag = True
    train = OrganAMNIST(split="train", download=download_flag, root="./data")
    test = OrganAMNIST(split="test", download=download_flag, root="./data")
    return train, test


def get_organmnist_coronal():
    download_flag = True
    train = OrganCMNIST(split="train", download=download_flag, root="./data")
    test = OrganCMNIST(split="test", download=download_flag, root="./data")
    return train, test


def get_organmnist_sagittal():
    download_flag = True
    train = OrganSMNIST(split="train", download=download_flag, root="./data")
    test = OrganSMNIST(split="test", download=download_flag, root="./data")
    return train, test
...
# ============================================
# *** ADDED: MedMNIST entries in DATASET_GETTERS ***
# ============================================
DATASET_GETTERS.update(
    {
        "medmnist": get_dermamnist,  # Default MedMNIST to DermaMNIST
        "MedMNIST": get_dermamnist,  # Case-insensitive support
        "dermamnist": get_dermamnist,
        "derma-mnist": get_dermamnist,
        "organmnist_axial": get_organmnist_axial,
        "organmnist_coronal": get_organmnist_coronal,
        "organmnist_sagittal": get_organmnist_sagittal,
    }
)
...
def fetch_data(dataset_name):
    if dataset_name not in DATASET_GETTERS:
        print(
            f'Dataset "{dataset_name}" not implemented. Please add it to the data manager.'
        )
        return None
...
def split_train_validation(train_data, val_fraction=0.1, seed=None):
    train, val = train_test_split(
        train_data, test_size=val_fraction, shuffle=True, random_state=seed
    )
...
def preprocess_subset(subset, index):
    ...
    labels = []
    if index == -1:
        desc = f"Processing data for test set"
    elif index == -2:
        desc = f"Processing data for val set"
    else:
        desc = f"Processing data for client: {index}"
...
def create_iid_dataset(
    dataset_name, num_clients=1, output_name="iid", val_fraction=0.1, seed=None
):
    if seed is not None:
        set_seed(seed)
    ...
    for index, subset in enumerate(
        train_subsets
    ):  # Note: This can be threaded if performance is lacking
...
def create_dirichlet_feature_skew(
    ...
    train_subsets = feature_skew_dirichlet(
        train, alpha_feat_split=alpha_feat_split, num_clients=num_clients, seed=seed
    )
...
def create_dirichlet_label_skew(
    ...
    train_subsets = label_skew_dirichlet(
        train, alpha_label_split=alpha_label_split, num_clients=num_clients, seed=seed
    )
...
def create_percentage_label_skew(
    ...
    train_subsets = label_skew_percentage(
        train, percentage_skew=percentage_skew, num_clients=num_clients
    )
...
def create_quantity_skew_dirichlet(
    ...
    train_subsets = quantity_skew_dirichlet(
        train, alpha_quant_split, num_clients=num_clients, seed=seed
    )
